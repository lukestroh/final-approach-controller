        ...
        for camera_type, metadata in self.sensor_config["cameras"].items():
            for i, frame in enumerate(metadata["tf_frames"]):
                ...
                if camera_type == "rgbd":
                    ...
                    self.sensor_data["cameras"][f"{metadata['name']}_{i}"] = []
...
def main():
    ...
    fapc_env.get_tof_observation(['p'])
    # print(fapc_env.tofs["vl6180_0"].film_coords)
